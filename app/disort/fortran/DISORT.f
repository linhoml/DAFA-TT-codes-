...
c     .. Parameters ..
     ...
     &          MXSQT
      PARAMETER ( MXCLY = 300, MXULV = 300, MXCMU = 300, MXUMU = 300,
...
c     .. Array Arguments ..
     ...
     &          UU( MAXUMU, MAXULV, MAXPHI ) , Factflux(MAXULV)
c     ..
c     .. Local Scalars ..
...
c     ..
      ...
      DATA      PASS1 / .TRUE. /, PRNTU0 / 1*.FALSE.,1*.FALSE. /
c      DATA      PASS1 / .TRUE. /, PRNTU0 / 1*.TRUE.,1*.FALSE. /
      DELTAM = .TRUE.
      CORINT = .TRUE.
...
c     IF( PRNT( 1 ) )
c     &    CALL PRTINP( NLYR, DTAUC, DTAUCP, SSALB, NMOM, PMOM, TEMPER,
c     &                 WVNMLO, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
c     &                 NPHI, PHI, IBCND, FBEAM, UMU0, PHI0, FISOT,
c     &                 LAMBER, ALBEDO, BTEMP, TTEMP, TEMIS, DELTAM,
c     &                 PLANK, ONLYFL, CORINT, ACCUR, FLYR, LYRCUT,
c     &                 OPRIM, TAUC, TAUCPR, MAXMOM, PRNT( 5 ) )
...
c     IF( PRNT( 3 ) .AND. .NOT.ONLYFL )
c     &    CALL PRTINT( UU, UTAU, NTAU, UMU, NUMU, PHI, NPHI, MAXULV,
c     &                 MAXUMU )
...
c------------------------ ⻯ͨ   2015.10.21-----------------------------
	      Factflux(LU)=Factflux(LU)+CWT( NN + 1 - IQ )*U0C(IQ,LU)
   30    CONTINUE
   ...
   50       CONTINUE
     ...
     &                   U0C( IQ, LU )
c------------------------ ⻯ͨ   2015.10.21-----------------------------
	      Factflux(LU)=Factflux(LU)+CWT( IQ - NN )*U0C(IQ,LU)
   60    CONTINUE

         Factflux(LU)= 2.*PI*Factflux(LU)    !diffuse fact
c     1	   +RFLDIR( LU )                   !add direct fact
...
c------------------------ for ICAS---------------------------------
c	do IQ=NN + 1, NSTR
c	 write(*,*) CMU(IQ - NN), U0C(IQ,1)
c	 write(100,*) U0C(IQ,1)
c	enddo
c------------------------------------------------------------------
...
c      write the intensity to the text is 50
               WRITE (*, '(2(0P,F16.4,F13.5,1P,E14.3))' )
     &            CMU(2*NN-IQ+1), U0C(IQ,LU),
     &           CMU(IQ),        U0C(IQ+NN,LU)
   90       CONTINUE
...
c     ..
   ...
   20 CONTINUE
     ...
     &                   WVNML, WVNMHI, NTAU, UTAU, NSTR, NUMU, UMU,
...
c         IF( PLANK ) WRITE( *, '(A,2F14.4,/,A,F10.2,A,F10.2,A,F8.4)' )
c     &       '    Thermal emission in wavenumber interval :', WVNMLO,
c     &       WVNMHI,
c     &       '    Bottom temperature =', BTEMP,
c     &       '    Top temperature =', TTEMP,
c     &       '    Top emissivity =', TEMIS
