...
try:
    from spectral_features import augmented_channel_count
except ImportError:
    augmented_channel_count = None
...
def lsga_hsi(args):
    ...
    if in_chans <= 0:
        base_channels = int(args["pca_num"])
        if augmented_channel_count is not None:
            in_chans = augmented_channel_count(base_channels, args)
        else:
